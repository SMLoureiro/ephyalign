...
class EpochMetrics:
    ...
    def __repr__(self) -> str:
        """Return string representation of EpochMetrics."""
        return (
...
def compute_epoch_metrics(
    ...
    peak_window_ms: float | None = None,
) -> EpochMetrics:
    ...
    peak_cv = (
        peak_std / abs(peak_mean) if peak_mean != 0 and not np.isnan(peak_mean) else float("nan")
    )
    ...
    logger.info(f"Computed metrics for {n_epochs} epochs: peak={peak_mean:.3f}±{peak_std:.3f}")

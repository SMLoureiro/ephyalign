...
def save_atf(
    ...
    with path.open("w") as f:
        ...
        # Column headers
        headers = ['"Time (ms)"'] + [f'"Sweep{i + 1} ({y_units})"' for i in range(n_epochs)]
        f.write("\t".join(headers) + "\n")

        # Data rows
        for i in range(epoch_length):
            row = [f"{time_ms[i]:.6f}"] + [f"{epochs[j, i]:.6f}" for j in range(n_epochs)]
            f.write("\t".join(row) + "\n")
...
def write_stats_report(
    ...
    with path.open("w") as f:
        ...
        ttp_mean = metrics.time_to_peak_mean_ms
        ttp_std = metrics.time_to_peak_std_ms
        f.write(f"Time to peak (ms): {ttp_mean:.3f} ± {ttp_std:.3f}\n")
        rt_mean = metrics.rise_time_mean_ms
        rt_std = metrics.rise_time_std_ms
        f.write(f"Rise time 10-90% (ms): {rt_mean:.3f} ± {rt_std:.3f}\n")
        ...
        for i in range(metrics.n_epochs):
            ...
            tp = (
                metrics.time_to_peak_s[i] * 1000
                if i < len(metrics.time_to_peak_s)
                else float("nan")
            )

...
def cli(ctx: click.Context, verbose: bool, debug: bool) -> None:
    r"""Ephyalign - Electrophysiology Response Alignment Toolkit.
...
def process_command(
    ...
    try:
        ...
        peak_mean = result.metrics.peak_amp_mean
        peak_std = result.metrics.peak_amp_std
        click.echo(f"  Peak amplitude: {peak_mean:.3f} ± {peak_std:.3f}")
...
def batch_command(
    ...
    r"""Process multiple ABF files in batch.
...
def info_command(input_file: str) -> None:
    r"""Display information about an ABF file.
    ...
    try:
        ...
        if info["channel_names"]:
            ...
            for i, (name, unit) in enumerate(
                zip(info["channel_names"], info["channel_units"], strict=False)
            ):
...
@click.option(
    "--format", "-f", type=click.Choice(["yaml"]), default="yaml", help="Config file format"
)
def init_config_command(output_file: str, format: str) -> None:
    r"""Generate an example configuration file.
...
def validate_command(config_file: str) -> None:
    r"""Validate a configuration file.
